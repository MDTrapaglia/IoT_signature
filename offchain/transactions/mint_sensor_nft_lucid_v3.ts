#!/usr/bin/env tsx
/**
 * Mint Sensor NFT using Lucid Evolution - Version 3
 * Testing different parameter serialization methods
 */

import { Blockfrost, Lucid, Data, applyParamsToScript, Constr, mintingPolicyToId, fromText } from "@lucid-evolution/lucid";
import dotenv from "dotenv";

dotenv.config();

// NFT minting policy code from plutus.json
const nft_code = "";

async function main() {
    const sensorId = process.argv[2] || "ESP32_01";

    console.log("=".repeat(70));
    console.log("Mint Sensor NFT - Testing Parameter Serialization");
    console.log("=".repeat(70));
    console.log(`Sensor ID: ${sensorId}`);

    // Initialize Lucid
    const lucid = await Lucid(
        new Blockfrost(
            "https://cardano-preprod.blockfrost.io/api/v0",
            process.env.BLOCKFROST_API_KEY || ""
        ),
        "Preprod"
    );

    // Load wallet
    const meshPrivateKey = process.env.PRIVATE_KEY;
    if (!meshPrivateKey) {
        throw new Error("PRIVATE_KEY not found in .env");
    }

    console.log("\n🔑 Loading wallet...");

    // Import lucid-cardano C module for key derivation
    const { C } = await import("lucid-cardano");
    const rootKey = C.Bip32PrivateKey.from_bech32(meshPrivateKey);
    const harden = (num: number) => 0x80000000 + num;
    const accountKey = rootKey
        .derive(harden(1852))
        .derive(harden(1815))
        .derive(harden(0));
    const paymentKey = accountKey.derive(0).derive(0).to_raw_key();
    const paymentKeyBech32 = paymentKey.to_bech32();

    // Select wallet
    (lucid as any).selectWallet.fromPrivateKey(paymentKeyBech32);

    const walletAddr = await (lucid as any).wallet().address();
    console.log("  ✅ Wallet loaded:", walletAddr.substring(0, 20) + "...");

    // Get UTXOs
    const utxos = await (lucid as any).wallet().getUtxos();

    if (!utxos || utxos.length === 0) {
        throw new Error("No UTXOs available in wallet");
    }

    const ownerUtxo = utxos[0];
    console.log(`\n📦 Selected UTXO: ${ownerUtxo.txHash}#${ownerUtxo.outputIndex}`);

    // Token name
    const token_name = `SENSOR_${sensorId}`;
    const tokenNameHex = fromText(token_name);

    console.log(`\n🧪 Testing different parameter serialization methods...`);

    // METHOD 1: Using hex string directly (current approach)
    console.log(`\n[Method 1] Hex string directly`);
    const tokenNameData1 = Data.to(tokenNameHex);
    console.log(`  Token Name Data: ${tokenNameData1}`);

    // METHOD 2: Using Bytes from hex
    console.log(`\n[Method 2] Using Data.Bytes`);
    try {
        // Convert hex to bytes
        const tokenNameBytes = Buffer.from(token_name, 'utf8');
        const tokenNameData2 = Data.to(tokenNameBytes.toString('hex'));
        console.log(`  Token Name Data: ${tokenNameData2}`);
    } catch (e: any) {
        console.log(`  ❌ Error: ${e.message}`);
    }

    // METHOD 3: Try without wrapping in Data.to for ByteArray
    console.log(`\n[Method 3] Direct hex (no Data.to wrapper)`);
    console.log(`  Token Name Data: ${tokenNameHex}`);

    // Let's try Method 3 - direct hex without Data.to
    console.log(`\n\n🔨 Building transaction with Method 3...`);

    const utxoRefData = Data.to(
        new Constr(0, [
            ownerUtxo.txHash,
            BigInt(ownerUtxo.outputIndex)
        ])
    );

    // Try passing tokenNameHex directly without Data.to()
    const mintingScript = applyParamsToScript(nft_code, [utxoRefData, tokenNameHex]);

    const mintingPolicy = {
        type: "PlutusV3" as const,
        script: mintingScript
    };

    const policyId = mintingPolicyToId(mintingPolicy);
    const nftUnit = policyId + tokenNameHex;

    console.log(`  Policy ID: ${policyId}`);
    console.log(`  NFT Unit: ${nftUnit}`);

    const redeemer = Data.to(new Constr(0, []));

    try {
        const tx = await lucid
            .newTx()
            .collectFrom([ownerUtxo])
            .attach.MintingPolicy(mintingPolicy)
            .mintAssets({ [nftUnit]: BigInt(1) }, redeemer)
            .complete();

        console.log(`  ✅ Transaction built successfully`);
        console.log(`  🔄 Signing...`);

        const signedTx = await tx.sign.withWallet().complete();

        console.log(`  🔄 Submitting...`);
        const txHash = await signedTx.submit();

        console.log(`\n✅ SUCCESS!`);
        console.log(`  Tx Hash: ${txHash}`);
        console.log(`  Explorer: https://preprod.cardanoscan.io/transaction/${txHash}`);
        console.log(`\n📋 NFT Details:`);
        console.log(`  Policy ID: ${policyId}`);
        console.log(`  Asset Name: ${tokenNameHex}`);
        console.log(`  Full Unit: ${nftUnit}`);
    } catch (error: any) {
        console.error(`\n❌ Method 3 failed: ${error.message}`);
        console.log(`\n🔨 Trying Method 1 (original approach with Data.to)...`);

        // Fallback to Method 1
        const tokenNameData = Data.to(tokenNameHex);
        const mintingScript2 = applyParamsToScript(nft_code, [utxoRefData, tokenNameData]);

        const mintingPolicy2 = {
            type: "PlutusV3" as const,
            script: mintingScript2
        };

        const policyId2 = mintingPolicyToId(mintingPolicy2);
        const nftUnit2 = policyId2 + tokenNameHex;

        const tx = await lucid
            .newTx()
            .collectFrom([ownerUtxo])
            .attach.MintingPolicy(mintingPolicy2)
            .mintAssets({ [nftUnit2]: BigInt(1) }, redeemer)
            .complete();

        console.log(`  ✅ Transaction built successfully`);
        const signedTx = await tx.sign.withWallet().complete();
        const txHash = await signedTx.submit();

        console.log(`\n✅ SUCCESS with Method 1!`);
        console.log(`  Tx Hash: ${txHash}`);
        console.log(`  Explorer: https://preprod.cardanoscan.io/transaction/${txHash}`);
    }
}

main().catch((error) => {
    console.error("\n❌ Fatal Error:", error.message || error);
    if (error.stack) {
        console.error("\n📚 Stack Trace:");
        console.error(error.stack);
    }
    process.exit(1);
});
